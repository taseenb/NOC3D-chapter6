...
class Vehicle {
  constructor (id, scene, position, maxspeed, maxforce, bounds) {
    ...
    this.mesh = this.createMesh(1.2, 0.7)
  ...
  /**
   * Calculate a steering force to align to other vehicles
   * @param vehicles
   */
  align (vehicles, idx) {
    if (!vehicles) return
    ...
    // Get sum of the velocities of all vehicles
  ...
  repel (predator) {
    const radius = 15
    const distance = this.position.distanceTo(predator)

    if (distance < radius) {
      const dir = vec3().subVectors(predator, this.position)
      const factor = distance / radius
      dir.normalize()
      dir.multiplyScalar(-(this.maxspeed + factor))
  ...
  applyBehaviors (vehicles, target) {
    ...
    const align = this.align(vehicles)
    const cohesion = this.cohesion(vehicles)
    const repel = this.repel(target)
    // const seek = this.seek(target)

    // Weight forces
    separate.multiplyScalar(1.5)
    align.multiplyScalar(1)
    cohesion.multiplyScalar(1)
    repel.multiplyScalar(10)
    // seek.multiplyScalar(0.1)
