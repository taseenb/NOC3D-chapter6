...
class App {
  ...
  init3dScene () {
    ...
    // Create vehicle
    this.vehicle = new Vehicle(this.scene, new THREE.Vector2(), 0.3, 0.005)
  ...
  initRaycaster () {
    ...
    // Where the camera plane should be looking at
    this.cameraPlaneLookAt = new THREE.Vector3()

    // Update the plane position, so it always faces the camera
    this.controls.addEventListener('change', (e) => {
      // console.log(e.target)
      this.cameraPlaneLookAt = this.camera.position
    })
  }
  ...
  render (now, delta) {
    // Update camera plane look at
    this.cameraPlane.lookAt(this.cameraPlaneLookAt)
